...
<div
    ...
    <div
        ...
        <form action="{{ route('servicios.store') }}" method="POST" class="mt-5 space-y-4">
            @csrf
            ...
            <div>
                ...
                <select id="id_direccion_municipal" name="id_direccion_municipal" class="w-full">
                    ...
                    <option value="{{ $d->id }}" {{ old('id_direccion_municipal') == $d->id ? 'selected' : '' }}>
                        {{ $d->nombre_direccion }}
                    </option>
                    @endforeach
                </select>
                @error('id_direccion_municipal')
                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <button
                    ...
                    class="inline-flex justify-center rounded-lg border border-gray-200 bg-white
                           ...
                           hover:bg-gray-50 focus:outline-none focus:ring-3 focus:ring-gray-500/10
                           dark:border-gray-700 dark:bg-gray-900
